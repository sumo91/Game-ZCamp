...
export function getWoodProductionPerSecond(state: WoodIncomeState, catalog: ContentCatalog = starterCatalog, heroDefinition?: HeroDefinition | null): number {
  ...
  const multiplier = state.hero ? (heroDefinition?.woodProductionMultiplier ?? getHeroDefinition(starterHeroContent, state.hero.definitionId)?.woodProductionMultiplier ?? 1) : 1;
